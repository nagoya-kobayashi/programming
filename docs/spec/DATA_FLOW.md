...
本書では、クライアントと Google Apps Script (GAS)、Pyodide ワーカー間でデータがどのように流れるかを時系列で説明します。ASCII 形式のシーケンス図とフロー図を用いて通信仕様を明確にします。各シートの列定義や並び順は `SPREADSHEET_SCHEMA.md` を参照してください。
...
- ユーザ進捗表 (user_progress): buildUserProgress_ が task シートの課題と各 <UserId> シートの score を集計し、属性順（基礎→演習→発展→その他）・Path 順に並べた課題列を持つ user_progress シートを生成する。1 行目に GeneratedAt/TaskCount、2 行目にヘッダ（UserId/ClassId/Number と属性別サマリ列、全体サマリ列、TaskId 列）、3 行目に課題 Attribute、4 行目に課題 Path を格納し、5 行目以降にユーザごとのクリア件数/総件数、スコア合計/満点合計、各課題のスコアを出力する。getUserProgress_ は既存シートを読み込み、無ければ再計算して返す。progress.html は POST action=getUserProgress/buildUserProgress でシートを取得し、100 点を属性色の★、数値スコアをそのまま表示、未採点は空欄で描画する。
- トップ画面のハイスコア: index.html はセッション有無にかかわらず action=getUserProgress を POST で呼び出し、キャッシュ済み user_progress シートのみを読み込む（再集計は呼ばない）。Attribute=その他を除外し、スコア合計降順で上位 20 件を抽出し、クラス・クリア件数/総数・スコア合計・基礎/演習/発展の課題スコア連結を表示する。100 点は属性色の★、数値スコアはそのまま、未提出・未採点・0 点は空欄。
