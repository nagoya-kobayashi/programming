...
本書はシステムのフロントエンドとバックエンドを開発・保守するための詳細な設計資料です。画面構成、主要な JavaScript 関数、データ保存の仕組み、認証方式、スプレッドシート構造、環境依存の制約などを網羅しています。ここに記載された情報を改変せずに利用し、改善時は影響範囲を十分に確認してください。
...
### grading.html???????
- ??: ???????????????????????????????/????????? `Submitted` ??????????????
- UI: ????????ID/???ID?????????????? `??100` ?????????????????????????????????????????????????????????????????????`submitted=true` ??????? Tab ???????????`score/comment` ?????????????`submitted=false` ?? `score/comment` ???????????????????????? `[???]` ??? `[???]` ??????????????Tab ??? `??100` ????????????????? `tabIndex=-1` ?????
- 出力が "[plot]" のときは grading.js で Pyodide と matplotlib を同期ロードし、提出コードをそのまま実行して得たグラフを横幅 50% のサムネイルで表示する（割り込みや入力待ちは考慮せず単純実行）
- ?????? `ALL` ????????????????????????`students` ???????????ID??????????????ID????????????????????????????
- ??????????????????????????????ON ????????????????????? Tab ??????????????????? `localStorage`?`grading.<server>.submittedOnly`?????????????????????????
- ??????????????????????????????`??100` ???????????????????????????????????????????
- ??????: `[???]` ? `[???]` ??????????????/??????????????????????????????????????????????????? `data-editing=true` ??????? `[???]` ????????????????? dirty ????????????????????
- ???????? task ?????????????????????????????????????????????????????????????????? order ???????????
- ?????: `action=getClassSubmissions` (POST) ? `tasks`?`students[{userId,number}]`?`submissions[userId][taskId]={code,output,hintOpened,submitted,score,comment}` ?????????ID???????????????????????
- ?????/????: ?????? `grading.<server>.cache.<ClassId>` ? tasks/students/submissions/fetchedAt ??????????????????? `lastLoadedAt` ??? fetchedAt ???? `getClassSubmissions` ??????Apps Script ? `<UserId>` ???? `SavedAt >= lastLoadedAt` ??????????????? `fetchedAt` ????????????????????????????`grading.<server>.userClass.<UserId>` ????ID??????????????
- ????: `??100` ???????????????? score=100 ????????????? `entries=[{userId,taskId,score,comment}]` ? JSON ????? `action=saveScores` ?????dirty ????? payload ??????Apps Script ? `<UserId>` ???? Score/Comment ??????`Submitted` ? FALSE ??????`submitted=false` ?????????????? `force:true` ??????????????? UI ??? `[???]` ????????score=100 ????? `.sparkle-star` ???????
- ????????: ???? (`score`/`comment`) ? `getUserSnapshot_` ? `loadSnapshot()` ????? `task_panel.js` ? `resultsData` ????????`resultsPath` ???????????????? `score` ??? `[???]` ??????????????????????????? `submitted=true` ???? `[???]` ????????
...
- データソース: Apps Script の getSubmissionSummary（キャッシュ読み込み）と uildSubmissionSummary（再集計）を利用。シートは TaskId/Title/Path に続けて <Class> クリア済 <Class> 採点済 <Class> 提出済 <Class> 未提出 を4列1組で並べ、1行目に GeneratedAt を保持します。
